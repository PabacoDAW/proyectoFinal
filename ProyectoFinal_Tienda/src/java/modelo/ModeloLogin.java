...
public class ModeloLogin {
    ...
    if (u != null && u.getCarrito() != null) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("ProyectoFinal_TiendaPU");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        Usuario usuarioConProductos = em.find(Usuario.class, u.getId());
        usuarioConProductos.getCarrito().getProductos().size();
        em.getTransaction().commit();
        em.close();
        emf.close();
        return usuarioConProductos.getCarrito().getProductos().size();
    }
    return 0;
}
